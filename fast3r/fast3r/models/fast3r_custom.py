...
class Fast3R(nn.Module,
    ...
    def forward(self, views, known_embeddings=None,known_positions=None,known_shapes = None,encoder_only = False,profiling=False):
        """
        Args:
        ...
        if num_known<len(views): #encoder only and normal 
            encoded_feats, positions, shapes = self._encode_images(views[num_known:])
        #shape must be the same!!!
        if encoder_only: 
            return encoded_feats,positions,shapes
        if  known_embeddings is not None and num_known<len(views): #normal situation 
            encoded_feats = tuple(known_embeddings) +encoded_feats
            positions = tuple(known_positions) +positions 
            shapes = len(encoded_feats)* (shapes[0],) 
        
        if num_known >= len(views):
            encoded_feats = tuple(known_embeddings) 
            positions = tuple(known_positions)
            shapes = len(encoded_feats)* (known_shapes[0],) 
        ...
        if profiling:
            return final_results,  encoded_feats, positions
        else:
            return final_results, encoded_feats, positions
