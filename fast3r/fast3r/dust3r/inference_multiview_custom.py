...
def loss_of_one_batch(
    batch, model, criterion, device, precision,feat_embeddings= None,pose_embeddings=None,shpae = None,encoder_only = False, symmetrize_batch=False, use_amp=False, ret=None, profiling=False,
):
    """
    Args:
    ...
    with torch.autocast(**autocast_dict):
        if profiling:
            preds,encoded_feats, positions, profiling_info = model(views,feat_embeddings,pose_embeddings,shpae,encoder_only = encoder_only, profiling=profiling)
        else:
            preds,encoded_feats, positions = model(views,feat_embeddings,pose_embeddings,shpae,encoder_only = encoder_only, profiling=profiling)
        if encoder_only == True:
            return preds,encoded_feats, positions  #  is feat pose and shape
    ...
    return result[ret] if ret else result ,encoded_feats, positions


@torch.inference_mode()
def inference_custom(multiple_views_in_one_sample,  model, device, dtype, feat_embeddings= None,pose_embeddings=None,shapes = None,encoder_only= False,verbose=True, profiling=False):
    ...
    res, encoded_feats, positions= loss_of_one_batch(
        collate_with_cat([tuple(multiple_views_in_one_sample)]), model, None, device, dtype,feat_embeddings,pose_embeddings, shapes,encoder_only= encoder_only, profiling=profiling
    )
    if encoder_only:
        return res, encoded_feats, positions  #feat position shape
    ...
    return result,encoded_feats, positions
