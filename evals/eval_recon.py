...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_pcd", default="/home/lingxianghu/SLAM3R/results/gt/replica_294518/office0_pcds.npy")
    parser.add_argument("--res_dir", default="/home/lingxianghu/VGGT-SLAM/replica_logs/run_office0/poses_logs/")
    parser.add_argument("--gt", default="/home/lingxianghu/datasets/Replica/office0/traj_tum.txt")
    parser.add_argument("--est", default="/home/lingxianghu/VGGT-SLAM/replica_logs/run_office0/poses.txt")
    ...
    all_points = []
    all_valid_flags = []  # 可选：记录每个点来自哪个 mask
# 按文件名中的第一个数字排序（如 0, 7, 40, 109...）
    npz_dir = args.res_dir
    # 获取所有 .npz 文件
    npz_files = glob.glob(os.path.join(npz_dir, "*.npz"))

    if not npz_files:
        print(f"❌ 在 {npz_dir} 中未找到 .npz 文件")
        exit(1)

    # 定义排序 key：提取文件名中的第一个数字
    def extract_step(filename):
        ...
        for p in parts:
            if p.isdigit():
                return int(p)
        print(f"⚠️ 无法解析文件名中的数字: {basename}")
        return float('inf')  # 排到最后

    # 按数字排序
    npz_files = sorted(npz_files, key=extract_step)

    # 打印顺序验证
    print(f"✅ 找到 {len(npz_files)} 个 .npz 文件，按数字顺序处理：")
    for i, file_path in enumerate(npz_files):
        #print(f"处理 [{i+1}/{len(npz_files)}]: {os.path.basename(file_path)}")
        with np.load(file_path) as data:
            pointcloud = data["pointcloud"]  # (294, 518, 3)
            mask = data["mask"]              # (294, 518)
        all_points.append(pointcloud)
        all_valid_flags.append(mask)



    pred_pcd = np.array(all_points)
    _,H,W = valid_masks.shape
    pred_confs = np.array(all_valid_flags)
    ...
    gt_pcd = pcd_gt[masks_valid]
    pred_pcd = pred_pcd[masks_valid]
